...
var kkpager = {
		...
		focus_gopage : function (){
			...
			btnGo.show();
			btnGo.css('left','0px');
			$('#'+this.gopageWrapId).css('border-color','#B44C2A');
			...
			setTimeout(function(){
				...
				btnGo.animate({
				    left: '-=44'
					  ...
					  btnGo.css('left','0px');
					  btnGo.hide();
					  $('#'+_this.gopageWrapId).css('border-color','#B44C2A');
				  });
			},400);
		},
		//跳转框页面跳转
		...
		generPageHtml : function(config,enforceInit){
			...
			if(this.isGoPage){
				gopage_info = '&nbsp;'+this.lang.gopageBeforeText+'<span id="'+this.gopageWrapId+'">'+
					'<input type="button" style="color:#B44C2A" id="'+this.gopageButtonId+'" onclick="kkpager.gopage()" value="'
